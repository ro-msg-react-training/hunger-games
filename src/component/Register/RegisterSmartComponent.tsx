...
import {
  ...
  loginFromRegiterEventHandler
} from "../../ReduxStore/RegisterSection/actions";
...
export interface RegisterComponentState {
  ...
  history: any;
  onKeyPressedWhileInputIsFocused: (
...
class RegisterSmartView extends React.Component<RegisterComponentState> {
  ...
  render() {
    return <RegisterDumpView {...this.props} />;
  }
}
...
const checkInputFields = (dispatch: Dispatch, userData: IUser): boolean => {
  ...
  if (userData.username.length === 0) {
    ...
    dispatch(
      showNotification(
        "Username should have a length comprised between 4 and 20 characters!",
        "warning"
      )
    );
    operationStatus = false;
    ...
    dispatch(
      showNotification(
        "The password should have a length comprised between 4 and 20 characters!",
        "warning"
      )
    );
    operationStatus = false;
    ...
    dispatch(
      showNotification("The email address should end in @msg.group!", "warning")
    );
    operationStatus = false;
  }
...
const mapStateToProps = (state: GlobalState) => ({
  ...
  activateNavbar:
    state.registerReducerGlobal.activateNavbar &&
    state.loginReducerGlobal.activateNavbar
      ? true
      : false //only if both the registration and login were ok
});
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  ...
  saveUserData: (props: RegisterComponentState) => {
    if (checkInputFields(dispatch, props.userInfo)) {
      if (!checkUser(props.userInfo)) {
        //checks for already existing user, in order to register the new account
        dispatch(
          showNotification(
            "User  " + props.userInfo.username + " created successfully!",
            "success"
          )
        );
        ...
        if (checkUser(props.userInfo)) {
          //check for existing user, in order to perform login
