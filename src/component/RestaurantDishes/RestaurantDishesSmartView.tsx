...
export interface RestaurantDishesState {
  ...
  generateDishesList: (
    ...
    dishesForCurrentRestaurant: IFood[]
  ) => ReactNode;
  addFoodToDemandsOnClick: (
    props: SingleDishState,
    selectedFood: IFood
  ) => void;
...
class RestaurantDishesSmartView extends React.Component<RestaurantDishesState> {
  ...
  render() {
    return <RestaurantDishesDumpView {...this.props} />;
  }
}

const getCurrentRestaurant = (
  props: RestaurantDishesState,
  resArray: IRestaurant[]
): IRestaurant => {
...
const getOpeningHour = (openingHours: string): Date => {
  ...
  openingHourString[0]
    .split(":")
    .map((time: string) => openingHoursStringSplit.push(time));
...
const getClosingHour = (closingHours: string): Date => {
  ...
  closingHourString[0]
    .split(":")
    .map((time: string) => closinggHoursStringSplit.push(time));
...
const isCurrentTimeInWorkingHours = (
  openingHours: string,
  closingHours: string
): boolean => {
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  loadCurrentRestaurantFoods: (props: RestaurantDishesState) => {
    ...
    let correspondingRestaurant: IRestaurant = getCurrentRestaurant(
      props,
      restaurantsArray
    );
  ...
  generateDishesList: (
    ...
    if (dishesForCurrentRestaurant.length === 0) {
      ...
      let singleDishProps: SingleDishState = {
        ...
        isOutsideOfWorkingHours: isCurrentTimeInWorkingHours(
          props.currentRestaurant.opening_hour,
          props.currentRestaurant.closing_hour
        ),
        incrementOrdersForRestaurant: props.incrementOrdersForRestaurant.bind(
          {} as any
        ),
      ...
      dishesForCurrentRestaurant.map((food: IFood) => {
        ...
        foodCards.push(
          <SingleDish
            {...singleDishProps}
            key={"Restaurant " + food.id_restaurant + ", food " + food.id_food}
          />
        );
        return true;
      });
