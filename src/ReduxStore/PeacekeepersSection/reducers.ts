...
export function peacekeepersReducer(
  state: PKTypes.PeacekeepersState = initialState,
  action: PKTypes.PeacekeepersActionTypes
): PKTypes.PeacekeepersState {
  switch (action.type) {
    case PKTypes.ADD_ORDER: {
      let ordersListWithTheNewOrder: IOrders[] = [
        ...state.placedOrders,
        action.newOrder
      ];
